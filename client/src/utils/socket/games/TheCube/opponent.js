    ...
    const animationEngine = ( () => {
      ...
      class World extends Animation {
        ...
        resize() {
          ...
          doc.documentElement.style.fontSize = docFontSize + 'px';
      ...
      class RoundedBoxGeometry extends THREE.BufferGeometry {
        ...
        doVertices() {
          ...
          var cornerOffset = new THREE.Vector3( this.edgeHalfWidth, this.edgeHalfHeight, this.edgeHalfDepth );
        ...
        doFaces() {
          ...
          var a = this.lastVertex;
          var b = this.lastVertex + this.cornerVertNumber;
      ...
      //doc.addEventListener( 'touchmove',  event => { event.preventDefault(); }, { passive: false } );
        ...
        constructor( element, options ) {
          ...
          this.drag = {
            ...
            start: ( event ) => {
              ...
              //win.addEventListener( ( this.touch ) ? 'touchmove' : 'mousemove', this.drag.move, false );
              //win.addEventListener( ( this.touch ) ? 'touchend' : 'mouseup', this.drag.end, false );
            ...
            move: ( event ) => {
              ...
              win.removeEventListener( ( this.touch ) ? 'touchmove' : 'mousemove', this.drag.move, false );
              win.removeEventListener( ( this.touch ) ? 'touchend' : 'mouseup', this.drag.end, false );
      ...
      class Controls {
        ...
        initDraggable() {
          ...
          this.draggable.onDragStart = position => {
            ...
            if ( edgeIntersect !== false && this.dragIntersect !== false ) {
              ...
              var x = Math.abs(edgeIntersect.point.x)
              var y = Math.abs(edgeIntersect.point.y)
              var z = Math.abs(edgeIntersect.point.z)
              var big = [x, y, z].sort()[2]
              this.dragNormal = new THREE.Vector3().setComponent([x, y, z].indexOf(big), 1);


              this.dragNormal2 = edgeIntersect.face.normal.round();
              this.flipType = 'layer';
          ...
          this.draggable.onDragMove = position => {
            ...
            if ( this.state === PREPARING && this.dragTotal.length() > 0.05 ) {
              ...
              if ( this.flipType === 'layer' ) {
                ...
                this.flipAxis = objectDirection.cross( this.dragNormal2 ).negate();
        ...
        rotateLayer( rotation, scramble, callback, flipAxis, flipLayer ) {
          ...
          const bounce = ( config == 2 ) ? this.bounceCube() : ( () => {} );
            ...
            if (!scramble) {
                this.flipAxis = JSON.parse(JSON.stringify(flipAxis))
                this.selectLayer(flipLayer)
            }
        ...
        rotateCube( rotation, callback, flipAxis ) {
        ...
        attach( child, parent ) {
          ...
          child.applyMatrix4( new THREE.Matrix4().copy( parent.matrixWorld ) );
      ...
      class Transition {
        ...
        complete( show ) {
          ...
          const text = this.game.dom.texts.complete;
          ...
          this.flipLetters( 'complete', letters, show );
      
          text.style.opacity = 1;
      
          const duration = this.durations[ 'complete' ];
      ...
      doc.querySelectorAll( 'range' ).forEach( el => {
      ...
      class Range {
        ...
        constructor( name, options ) {
          ...
          this.element = doc.querySelector( '.range[name="' + name + '"]' );
      ...
      class Storage {
        ...
        constructor( game ) {
          ...
          if ( ! userVersion || userVersion !== win.gameVersion ) {
      
            this.clearGame();
            this.clearPreferences();
            this.migrateScores();
            localStorage.setItem( 'theCube_version', win.gameVersion );
        ...
        //   const preferences = {
        //     cubeSize: prefs.cubeSize,
        //     flipConfig: prefs.flipConfig,
        //     dificulty: prefs.dificulty,
        //     fov: prefs.fov,
        //     theme: prefs.theme,
        //     colors: prefs.colors,
        //   };
      ...
      class IconsConverter {
        ...
        constructor( options ) {
          ...
          this.svgTag = doc.createElementNS( 'http://www.w3.org/2000/svg', 'svg' );
        ...
        convertAllIcons() {
          ...
          doc.querySelectorAll( this.tagName ).forEach( icon => { this.convertIcon( icon ); } );
      
        }
      
        convertIcon( icon ) {
      
          const svgData = this.icons[ icon.dataset.type ];
      ...
      class Game {
        ...
        constructor() {
          ...
          this.dom = {
            ui: doc.querySelector( '.ui' ),
            game: doc.querySelector( '.ui__game' ),
            back: doc.querySelector( '.ui__background' ),
            prefs: doc.querySelector( '.ui__prefs' ),
            theme: doc.querySelector( '.ui__theme' ),
            stats: doc.querySelector( '.ui__stats' ),
            texts: {
              title: doc.querySelector( '.text--title' ),
              note: doc.querySelector( '.text--note' ),
              timer: doc.querySelector( '.text--timer' ),
              complete: doc.querySelector( '.text--complete' ),
              best: doc.querySelector( '.text--best-time' ),
              theme: doc.querySelector( '.text--theme' ),
            },
            buttons: {
              prefs: doc.querySelector( '.btn--prefs' ),
              back: doc.querySelector( '.btn--back' ),
              stats: doc.querySelector( '.btn--stats' ),
              reset: doc.querySelector( '.btn--reset' ),
              theme: doc.querySelector( '.btn--theme' ),
            },
          };
          ...
          this.dom.texts.note.innerHTML = "Not Ready"
          ...
          socket.on('opponentReady', () => {    
            ...
            this.dom.texts.note.innerHTML = ""
            this.dom.texts.note.innerHTML = "Ready"             
           })

           socket.on('opponentInit', ({ moves }) => {
            this.scrambler.scramble({ moves });
           })
           ...
           socket.on('allReady', () => {                  
            ...
            socket.on('opponentLayer', ({ rotation, flipAxis, flipLayer }) => {
                this.controls.rotateLayer(rotation, false, () => {this.controls.checkIsSolved()}, flipAxis, flipLayer)
            })

            socket.on('opponentCube', ({ rotation, flipAxis }) => {
              this.controls.rotateCube(rotation, () => {}, flipAxis)
          })

            socket.on('opponentPref', ( preferences ) => {
                this.storage.loadPreferences(preferences)
                this.preferences.ranges.size.value = (JSON.parse(preferences)).cubeSize
                this.cube.resize(false)
            })
