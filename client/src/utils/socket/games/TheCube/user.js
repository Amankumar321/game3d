...
export default function start(index = 0) {
...
const animationEngine = ( () => {
  ...
  class Controls {
    ...
    rotateLayer( rotation, scramble, callback ) {
      ...
      if (!scramble){
        var totalAngle = rotation + this.flipAngle
        if (room_id) {
          socket.emit('rotateLayer', { rotation: totalAngle, flipAxis: this.flipAxis, flipLayer: this.flipLayer })
        }
      }
    ...
    rotateCube( rotation, callback ) {
      ...
      var totalAngle = rotation + this.flipAngle
      if (room_id) {
        socket.emit('rotateCube', { rotation: totalAngle, flipAxis: this.flipAxis })
      }
  ...
  class Storage {
    ...
    savePreferences() {
      ...
      if (room_id) {
        socket.emit('savePref',  JSON.stringify(preferences) )
      }
  ...
  class Game {
    ...
    allReadyFn = () => {
      var _scrambler = this.scrambler.scramble()
      var moves = _scrambler.moves
      if (room_id) {
        socket.emit('initMoves', { moves })
      } 
    ...
    initActions() {
      ...
      this.dom.texts.note.innerHTML = "Double tap to Start"
