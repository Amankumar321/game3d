...
export default function start(index = 0){
    ...
    function startBtnHandler2() {
        if(doc.getElementById('playAlert')) {
            doc.getElementById('playAlert').innerHTML = "Play";
        }
        setTimeout(function() {
            if (settings.platform == "mobile") {
                try {
    ...
    async function addNewBlock(blocklane, color, iter, distFromHex, settled) { //last two are optional parameters
        setTimeout(() => {
            if (room_id) {
                socket.emit('newBlock', {blocklane, color, iter, distFromHex, settled});  
            }
        }, 50)
        ...
        this.addBlock = function(block) {
            ...
            for (i = 0; i < MainHex.blocks.length; i++) {
                for (j = 0; j < MainHex.blocks[i].length; j++) {
                    var blk = MainHex.blocks[i][j];
                    blockArr.push({i: i, j: j, lane: blk.attachedLane, fallingLane: blk.fallingLane, iter: blk.iter, color: blk.color, targetAngle: blk.targetAngle, angle: blk.angle, dist: blk.distFromHex, hexAngle: this.targetAngle, hexPos: this.position})
                }
            }

            if (room_id) {
                socket.emit('settledBlocks', {blocks: blockArr, newScore: score})
            }
        };
...
//remember to update history function to show the respective iter speeds
    ...
    function drawScoreboard() {
        ...
        var txt = readyState === true ? 'Ready' : 'Play!'
        ...
        if (gameState === 0) {
            ...
            renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h + 10, fontSize, "rgb(44,62,80)", txt);
        } else if (gameState != 0 && textOpacity > 0) {
            textOpacity -= 0.05;
            renderText(trueCanvas.width / 2 + gdx + (scoreSize/15)*settings.scale, trueCanvas.height / 2 + gdy + (scoreSize/20)*settings.scale, fontSize, "rgb(236, 240, 241)", String.fromCharCode("0x25B6"), 'px Arial');
            renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2 + gdy - 155 * settings.scale, 100, "#2c3e50", "Hextris");
            renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h, fontSize, "rgb(44,62,80)", txt);
