...
import './MaskPass'
import './CopyShader'
import './HorizontalTiltShiftShader'
...
var responsiveScale = window.innerWidth >= 960 ? 1.5 : 1  ///custom

var camera = new THREE.PerspectiveCamera( 45, responsiveScale*window.innerWidth / window.innerHeight, .01, 100000 );
camera.position.set( 1, 1, -1.4 );
camera.lookAt( scene.position );

var renderer = new THREE.WebGLRenderer( { antialias: true, alpha: true } );
renderer.setSize( responsiveScale*window.innerWidth, window.innerHeight );
renderer.shadowMap.enabled = true;
...
window.addEventListener("resize", function() {
  ...
  renderer.setSize(responsiveScale*width, height);
  camera.aspect = responsiveScale*width / height;
...
class JoyStick{
  constructor(options){
    ...
    circle.style.cssText = "position:absolute; bottom:35px; width:80px; height:80px; background:rgba(126, 126, 126, 0.5); opacity: 0.5; border:#666666 solid medium; border-radius:50%; left:50%; transform:translateX(-50%);";
    const thumb = document.createElement("div");
    thumb.style.cssText = "position: absolute; left: 20px; top: 20px; width: 40px; height: 40px; border-radius: 50%; background: #e2e2e2;";
    ...
    if ('ontouchstart' in window){
      document.ontouchmove = function(evt){ 
            ...
            if (window.innerWidth < 960) {
                parent.document.getElementById('contentDivHome').style.opacity = 0.0
            }
        };
      document.ontouchend =  function(evt){
            joystick.up(evt);
            parent.document.body.style.overflow = 'auto'
            parent.document.getElementById('contentDivHome').style.opacity = 1
        };
    }
    else{
